...
import { checkUser, checkVendor } from "../../middlewares/authMiddleware";
...
// Get order info
app.openapi(Get({ path: "/order/info", query: z.object({ id: z.string() }), tags: ["Order"], middleware: [checkVendor] }), orderControllers().getOrderInfo);
// Modify order by super admin
app.openapi(Patch({ path: "/order/modify", query: z.object({ id: z.string() }), tags: ["Order"], middleware: [checkVendor], schema: OrderPostSchema }), orderControllers().adminModifyOrder);
